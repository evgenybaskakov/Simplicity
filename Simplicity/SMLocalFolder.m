...
- (void)updateMessages:(NSArray*)imapMessages remoteFolder:(NSString*)remoteFolderName updateDatabase:(Boolean)updateDatabase {
    ...
    [SMNotificationsController localNotifyMessagesUpdated:_localName updateResult:updateResult account:(SMUserAccount*)_account];
}
...
- (void)fetchMessageThreadsHeadersFromDescriptor:(SMMessageThreadDescriptor*)threadDesc {
    SMMessageThread *messageThread = [_messageStorage messageThreadById:threadDesc.threadId];
...
- (void)finishHeadersSync:(Boolean)updateDatabase {
    ...
    SMMessageStorageUpdateResult updateResult = [_messageStorage endUpdateWithRemoteFolder:_remoteFolderName removeVanishedMessages:YES updateDatabase:updateDatabase unseenMessagesCount:&_unseenMessagesCount processNewUnseenMessagesBlock:shouldUseNotifications? ^(NSArray *newUnseenMessages) {
    ...
    [SMNotificationsController localNotifyMessageHeadersSyncFinished:_localName hasUpdates:hasUpdates account:(SMUserAccount*)_account];
}
...
- (void)addMessage:(SMMessage*)message externalMessage:(BOOL)externalMessage updateDatabase:(BOOL)updateUpdate {
    if([_messageStorage addMessageToStorage:message updateDatabase:updateUpdate]) {
        ...
        [SMNotificationsController localNotifyMessagesUpdated:_localName updateResult:SMMesssageStorageUpdateResultStructureChanged account:(SMUserAccount*)_account];
    }
}
...
- (void)setMessageUnseen:(SMMessage*)message unseen:(Boolean)unseen {
    ...
    [SMNotificationsController localNotifyMessageFlagsUpdates:_localName account:(SMUserAccount*)_account];
...
- (Boolean)moveMessage:(uint32_t)uid threadId:(uint64_t)threadId useThreadId:(Boolean)useThreadId toRemoteFolder:(NSString*)destRemoteFolderName {
    ...
    if(useThreadId) {
        needUpdateMessageList = [_messageStorage deleteMessageFromStorage:uid threadId:threadId remoteFolder:_remoteFolderName unseenMessagesCount:&_unseenMessagesCount];
